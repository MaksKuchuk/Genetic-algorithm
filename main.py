...
class Agent:
	 def __init__(self, xabs, yabs, steps, power, screen):
	 	...
	 	self.power = power
	 	self.screen = screen
	 	self.mutOfVectors = 15
	 	self.selOfVectors = 10
...
class App:
	...
	def run(self):
		while True:
			...
			pg.display.set_caption(f'Generation: {self.gen}   Average: {self.average :.0f}   Best: {self.best :.0f}   FPS: {self.clock.get_fps() :.0f}')
